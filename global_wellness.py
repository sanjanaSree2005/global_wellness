...
if st.session_state["view"] is None and st.session_state["role"] is None:
    ...
    with col2:
        ...
        user_btn = st.button("👤 Login as User")
        admin_btn = st.button("🧑‍💼 Login as Admin")
...
#  ADMIN DASHBOARD
    ...
    if page == "dashboard":
        ...
        box_style = "background:#5a67d8; color:white; padding:10px; border-radius:10px; text-align:center;"
        with c1:
            st.markdown(f"<div style='{box_style}'><b>Users</b><br>{total_users}</div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div style='{box_style}'><b>Queries</b><br>{total_queries}</div>", unsafe_allow_html=True)
        with c3:
            st.markdown(f"<div style='{box_style}'><b>Knowledge Base</b><br>{total_kb}</div>", unsafe_allow_html=True)
        with c4:
            st.markdown(f"<div style='{box_style}'><b>Feedback</b><br>{total_fb}</div>", unsafe_allow_html=True)

        st.markdown("---")
        st.subheader("📈 Queries over last 14 days")
        ...
        ax.plot(labels, counts, marker='o', color="#2563eb", linewidth=1.8, markersize=3)
        ax.fill_between(labels, counts, color="#93c5fd", alpha=0.25)
        ...
        ax.set_title("Last 14 Days", fontsize=10, pad=8)
        ax.grid(alpha=0.25)
        ...
        if total_fb == 0:
            ...
            fig2, ax2 = plt.subplots(figsize=(1.3, 0.9))
            ...
            ax2.axis("equal")
            ax2.set_title("Feedback Summary", fontsize=4, pad=8)
            fig2.tight_layout()
            st.pyplot(fig2)
